<template>
  <div class="body">
      ...
      <div class="register-body" v-show="!jump">
        ...
        <div class="register-input">
          ...
          <input
            ...
            placeholder="8-12位密码，字母/数字/符号至少两种"
          />
          <img
            class="pwdeye"
            title="点击隐藏或显示密码"
        ...
        <div class="agreement">
          <input
            type="checkbox"
            id="agreement-check"
            style="width:14px;height:14px;background:rgba(115,215,225,1);border-radius:1px;"
          />
          <p>
            我已阅读并同意
            <a href="#">《开放平台用户注册协议》</a>
    ...
    <div class="foot-font">
      <p>Copyright© 2018赞同科技|京ICP备 14027589号-1</p>
    </div>
  </div>
</template>
<script>
import dragVerify from "vue-drag-verify"; //滑动解锁插件
import { Register, TestCode } from "../request/api.js"; // 导入api接口
...
export default {
  ...
  data() {
    return {
      ...
      openeye: require("@/assets/images/icon-visible.png"), //图片地址
      nopeneye: require("@/assets/images/icon-invisible.png"), //图片地址
      userPassword: "",
      //跳转属性
      jump: false
    };
    ...
    goLogin() {
      if (this.validateVcode()) {
        ...
        Register(content)
          ...
          .catch(err => {
            this.$Message.error('errortest',err);
          });
      } else {
        this.validateVcode();
      }
