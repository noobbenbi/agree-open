...
//此处import到时候要改
import { AppendApp } from "../request/api.js"; // 导入api接口
...
export default {
  ...
  created() {
    this.developerId = sessionStorage.getItem("devId");
  },
  methods: {
    handleSubmit(name) {
    ...
    async AppendApp() {
      var appType = parseInt(this.formValidate.appType);
      var formValidate = {
        appType: appType,
        appName: this.formValidate.appName,
        appDesc: this.formValidate.appTag,
        appTag: this.formValidate.appDesc,
        // developerId: this.developerId,
      };
      AppendApp(formValidate)
        .then(res => {
